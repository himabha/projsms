...
class ResellerController extends \yii\web\Controller
{
    ...
    */
    public function actionSmsNumbers()
    {
        ...
        $dataProvider = $searchModel->search($queryParams, $mysubusr, $search);
    ...
    */
    ...
    public function actionTdrExport()
    {
        ...
        if (isset($_SERVER['QUERY_STRING'])) {
            ...
            $agent_name = 'None';
            if (isset($params['agent_id']) && trim($params['agent_id']) != "") {
                if ($params['agent_id'] == 0) {
                    $agent_name = 'Un-located';
                } else {
                    $obj = User::findOne(intval($params['admin_id']));
                    if (isset($obj->username)) $agent_name = $obj->username;
                }
            }
                ...
                foreach ($headers as $hk => $hv) {
                    switch ($hv) {
                        ...
                        case "Bill Group":
                            ...
                            break;
                        case "Agent":
                            $sheet->setCellValueByColumnAndRow($hk + 1, $row, isset($v->users) ? $v->users->username : "");
                            $temp[$hk] = isset($v->users) ? $v->users->username : "";
                            break;
                        case "Delivered Time":
        ...
        if (\Yii::$app->request->queryParams['mode'] == 'csv') {
            ...
            header('Content-Disposition: attachment; filename="tdr_report.csv"');
            ob_end_clean();
            ...
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment; filename="tdr_report.xlsx"');
        ...
        if (isset($_SERVER['QUERY_STRING'])) {
            ...
            if (isset($params['agent_id']) && trim($params['agent_id']) != "") {
                if ($params['agent_id'] == 0) {
                    ...
                    $obj = User::findOne(intval($params['agent_id']));
    ...
    public function actionTestNumbers()
    {
        ...
        $mysubusr = User::find()->select('id')->where(['reseller_id' => Yii::$app->user->identity->id, 'role' => 2]);
        //$summary = $model->getSummary($mysubusr, false, false, true);
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams, $mysubusr, $search, User::isUserAdmin(\Yii::$app->user->id), true);
        $dataProvider->pagination->pageSize = $filter;
        $billgroups = Billgroup::getBillgroupItems();
